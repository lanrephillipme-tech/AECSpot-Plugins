...
namespace CopyFilterInstaller
{
    ...
    {
        // Revit 2024 internal-test package. This version intentionally supersedes prior diagnostic builds.
        public static string versionValue = "1.0.3";
        static void Main()
        {
            var project = new ManagedProject($"AEC Spot - Manage Filters {versionValue}",
                              new Dir(@"CommonAppDataFolder\Autodesk\ApplicationPlugins",
                                  new Dir(@"FilterManager.bundle",
                                  new File(@"..\FilterManager\PackageContents.xml"),
                                      new Dir(@"Contents",
                                       new Dir(@"2024",
                                            new Dir(@"Resources", new Dir(@"Images", new Files(@"..\FilterManager\bin\Debug\2024\Resources\Images\*.png"))),
                                            new File(@"..\FilterManager\FilterManager.addin"),
                                            new File(@"..\FilterManager\bin\Debug\2024\FilterManager.dll",
                                                new FilePermission("Everyone", GenericPermission.All)),
                                            new File(@"..\FilterManager\bin\Debug\2024\AECSpot.dll",
                                                new FilePermission("Everyone", GenericPermission.All)),
                                             new File(@"..\FilterManager\bin\Debug\2024\Xceed.Wpf.AvalonDock.dll",
                                                new FilePermission("Everyone", GenericPermission.All)),
                                             new File(@"..\FilterManager\bin\Debug\2024\Xceed.Wpf.AvalonDock.Themes.Aero.dll",
                                                new FilePermission("Everyone", GenericPermission.All)),
                                             new File(@"..\FilterManager\bin\Debug\2024\Xceed.Wpf.AvalonDock.Themes.Metro.dll",
                                                 new FilePermission("Everyone", GenericPermission.All)),
                                             new File(@"..\FilterManager\bin\Debug\2024\Xceed.Wpf.AvalonDock.Themes.VS2010.dll",
                                                 new FilePermission("Everyone", GenericPermission.All)),
                                             new File(@"..\FilterManager\bin\Debug\2024\Xceed.Wpf.Toolkit.dll",
                                                 new FilePermission("Everyone", GenericPermission.All))
            ...
            project.GUID = new Guid("1332603E-786B-4926-A05B-37742C3D6C50");
    ...
    public class CustomActions
    {
        ...
        public static ActionResult CheckRevitProcess(Session session)
        {
            try
            {
                ...
                if (!System.IO.File.Exists($"{appDataPath}\\Autodesk\\ApplicationPlugins\\FilterManager.bundle\\PackageContents.xml"))
              // if (!System.IO.File.Exists($"CommonAppDataFolder\\Autodesk\\ApplicationPlugins\\FilterManager.bundle\\PackageContents.xml"))
                {
                    //MessageBox.Show(System.IO.File.Exists($"{appDataPath}\\Autodesk\\ApplicationPlugins\\FilterManager.bundle\\PackageContents.xml").ToString(), $"{appDataPath}\\Autodesk\\ApplicationPlugins\\FilterManager.bundle\\PackageContents.xml");
                    return ActionResult.Success;
                }
                else
                {
                ...
                if (processes.Length > 0)
                {
                    ...
                    if (result == DialogResult.Yes)
                    {
                        ...
                        string basePath = Environment.ExpandEnvironmentVariables("CommonAppDataFolder\\Autodesk\\ApplicationPlugins\\FilterManager.bundle\\Contents");
                        List<string> filePaths = new List<string>();

                        for (int year = 2018; year <= 2025; year++)
                        {
                            //string filePath = $"{basePath}\\{year}\\FilterManager.dll";
                            //bool overwrittable = WaitForFilesToBeOverwritable(filePath);
                            //if (!overwrittable)
                            //{
                            //    MessageBox.Show($"The file FilterManager.dll {year} is still in use", "Warning");
